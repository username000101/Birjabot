...
void birja::command::add_god(icu::UnicodeString& command, TgBot::Message::Ptr message) {
    ...
    if (std::find(gods.begin(), gods.end(), message->from->id) == gods.end()) {
        LOG(__FUNCTION__, ": Attempt to mark the user: unathorized");
        global_bot->getApi().sendMessage(message->chat->id, "Вы не обладаете правами для выполнения данной команды", nullptr, reply_p);
        return;
    }

    if (message->replyToMessage) {
        global_bot->getApi().sendMessage(message->chat->id, db::add_god(message->chat->id, message->replyToMessage->from->id).message, nullptr, reply_p);
        return;
    }
    ...
    LOG(__FUNCTION__, ": Attempt to mark the ", username, " as god");
    global_bot->getApi().sendMessage(message->chat->id, db::add_god(message->chat->id, username).message, nullptr, reply_p);
