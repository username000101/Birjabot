#include "Command.hpp"

#include <algorithm>
#include <sstream>

#include <unicode/unistr.h>

#include "DBUtils.hpp"
#include "Utils.hpp"

void birja::command::open_wallet(icu::UnicodeString& command, TgBot::Message::Ptr message) {
    auto reply_p = std::make_shared<TgBot::ReplyParameters>();
    reply_p->messageId = message->messageId;
    reply_p->chatId = message->chat->id;

    if (command.length() < 18) {
        LOG(__FUNCTION__, ": Attempt to get balance of the user self");
        auto get_user_result = db::get_user(message->chat->id, message->from->id);
        if (!get_user_result.ok)
            global_bot->getApi().sendMessage(message->chat->id, get_user_result.message, nullptr, reply_p);
        else
            global_bot->getApi().sendMessage(message->chat->id, "Ваш баланс: " + std::to_string(std::get<User>(get_user_result.value).balance), nullptr, reply_p);

    } else {
        auto gods_result = db::get_gods(message->chat->id);
        if (!gods_result.ok) {
            LOG(__FUNCTION__, ": Failed to preparing command: gods_result.ok != true");
            global_bot->getApi().sendMessage(message->chat->id, gods_result.message, nullptr, reply_p);
            return;
        }

        auto gods = std::get<std::vector<std::int64_t>>(gods_result.value);
        if (std::find(gods.begin(), gods.end(), message->from->id) == gods.end()) {
            LOG(__FUNCTION__, ": Attempt to open the wallet of the user: unathorized");
            global_bot->getApi().sendMessage(message->chat->id, "Вы не обладаете правами для выполнения данной команды", nullptr, reply_p);
            return;
        }

        std::string stream_command;
        command.tempSubString(16).toUTF8String(stream_command);
        std::istringstream stream(stream_command);
        std::string username;
        stream >> username;

        auto get_user_result = db::get_user(message->chat->id, username);
        if (!get_user_result.ok)
            global_bot->getApi().sendMessage(message->chat->id, get_user_result.message, nullptr, reply_p);
        else
            global_bot->getApi().sendMessage(message->chat->id, "Баланс пользователя " + (username.starts_with("@") ? username : "@" + username) + ": " + std::to_string(std::get<User>(get_user_result.value).balance), nullptr, reply_p);
    }
}
